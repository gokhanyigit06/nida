...
import { ArrowLeft, Loader2, Upload, AlertCircle, CheckCircle2, Save, Trash2, LayoutGrid, Plus, ChevronUp, ChevronDown } from "lucide-react";
...
export default function EditProjectPage() {
    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            try {
                const docSnap = await getDoc(doc(db, "portfolio", projectId));
                if (docSnap.exists()) {
                    ...
                    setBgColor(data.bgColor || "#fbc9f2");
                    setTextColor(data.textColor || "black");
                    if (data.mediaTypes) setMediaTypes(prev => ({ ...prev, ...data.mediaTypes }));
                    ...
                    if (data.extraMediaRows) {
                        setExtraRows(data.extraMediaRows.map((row: any) => {
                            const itemCount = row.layout === "single" ? 1 : row.layout === "double" ? 2 : 3;
                            return {
                                id: row.id || Math.random().toString(36).substr(2, 9),
                                layout: row.layout,
                                files: Array(itemCount).fill(null),
                                previews: row.items.map((it: any) => it.url),
                                mediaTypes: row.items.map((it: any) => it.type),
                                existingUrls: row.items.map((it: any) => it.url)
                            };
                        }));
                    }
                } else {
                    setStatus({ type: "error", msg: "Proje bulunamadı!" });
                }
            } catch (error) {
                setStatus({ type: "error", msg: "Proje bilgileri alınamadı." });
            } finally {
                setFetching(false);
            }
        };
    ...
    const addExtraRow = (layout: "single" | "double" | "triple" | "triple-vertical") => {
        ...
        setExtraRows([...extraRows, {
            ...
            existingUrls: Array(itemCount).fill("")
        }]);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (!title || !category) throw new Error("Başlık ve kategori zorunludur!");
            ...
            const projectData = {
                ...
                extraMediaRows: await Promise.all(extraRows.map(async (row) => {
                    const items = await Promise.all(row.previews.map(async (prevUrl: string, idx: number) => {
                        const file = row.files[idx];
                        if (file) {
                            const url = await uploadFile(file, `${folderName}/extra`);
                            return { url, type: row.mediaTypes[idx] };
                        } else if (row.existingUrls?.[idx]) {
                            return { url: row.existingUrls[idx], type: row.mediaTypes[idx] };
                        }
                        return null;
                    }));
            ...
            await updateDoc(doc(db, "portfolio", projectId), projectData);
            setStatus({ type: "success", msg: "Proje başarıyla güncellendi! ✨" });
    ...
    if (fetching) return (
        <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center p-6">
            ...
            <span className="font-bold tracking-widest uppercase text-sm">PROJE BİLGİLERİ YÜKLENİYOR...</span>
        </div>
    );
        ...
        <div className="min-h-screen bg-[#050505] text-white p-6 md:p-12 font-sans overflow-x-hidden">
            <header className="flex items-center gap-6 mb-12 w-full max-w-[1000px] mx-auto">
                ...
                <h1 className="text-3xl md:text-5xl uppercase tracking-wider" style={{ fontFamily: 'var(--font-bebas-neue)' }}>
                    PROJEYİ <span className="text-white/40">DÜZENLE</span>
                </h1>
            </header>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-10">
                    ...
                    <div className="bg-[#0a0a0a] border border-white/10 rounded-[2rem] p-8 md:p-10 flex flex-col gap-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-2 md:col-span-2">
                                ...
                                <input value={title} onChange={e => setTitle(e.target.value)} required className="bg-black border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-white/40 transition-colors uppercase text-xl" style={{ fontFamily: 'var(--font-bebas-neue)' }} />
                            </div>
                            <div className="flex flex-col gap-2">
                                <label className="text-xs uppercase tracking-widest text-white/50 font-bold ml-1">Kategori *</label>
                                <input value={category} onChange={e => setCategory(e.target.value)} required className="bg-black border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-white/40 transition-colors uppercase" />
                            </div>
                            <div className="flex flex-col gap-2">
                                <label className="text-xs uppercase tracking-widest text-white/50 font-bold ml-1">Yıl</label>
                                <input value={year} onChange={e => setYear(e.target.value)} className="bg-black border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-white/40 transition-colors" />
                            </div>
                            <div className="flex flex-col gap-2">
                                <label className="text-xs uppercase tracking-widest text-white/50 font-bold ml-1">Kutu Arkaplan Rengi (#hex)</label>
                                <input value={bgColor} onChange={e => setBgColor(e.target.value)} className="bg-black border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-white/40 transition-colors" />
                            </div>
                            <div className="flex flex-col gap-2">
                    ...
                    <div className="bg-[#0a0a0a] border border-white/10 rounded-[2rem] p-8 md:p-10 flex flex-col gap-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-2">
                                ...
                                <input value={client} onChange={e => setClient(e.target.value)} className="bg-black border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-white/40 transition-colors" />
                            </div>
                            <div className="flex flex-col gap-2">
                                <label className="text-xs uppercase tracking-widest text-white/50 font-bold ml-1">Hizmetler (Virgülle ayırın)</label>
                                <input value={services} onChange={e => setServices(e.target.value)} placeholder="VIDEO EDITING, COLOR GRADING" className="bg-black border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-white/40 transition-colors" />
                            </div>
                            <div className="flex flex-col gap-2 md:col-span-2">
                                ...
                                <textarea value={description} onChange={e => setDescription(e.target.value)} rows={5} className="bg-black border border-white/10 rounded-xl px-5 py-4 text-white focus:outline-none focus:border-white/40 transition-colors resize-y leading-relaxed" />
                            </div>
                        </div>
                    </div>
                                ...
                                {extraRows.map((row, rowIdx) => (
                                    <div key={row.id} className="relative p-8 bg-white/[0.02] rounded-[2.5rem] border border-white/5 group/row hover:border-white/10 transition-colors">
                                        ...
                                        <div className={`grid gap-5 ${row.layout === "single" ? "grid-cols-1" : row.layout === "double" ? "grid-cols-2" : "grid-cols-3"}`}>
                                            {row.previews.map((prev: string, idx: number) => (
                                                <label key={idx} className={`relative w-full overflow-hidden bg-black border border-dashed border-white/10 rounded-2xl hover:border-white/40 cursor-pointer flex flex-col items-center justify-center group/item ${row.layout === "triple-vertical" ? "aspect-[9/16]" : row.layout === "single" ? "aspect-[21/9]" : "aspect-square"}`}>
                                                    ...
                                                    {prev ? (
                                                        row.mediaTypes[idx] === "video"
                                                            ? <video src={prev} autoPlay muted loop className="w-full h-full object-cover group-hover/item:opacity-40 transition-opacity" />
                                                            : <Image src={prev} alt="preview" fill className="object-cover group-hover/item:opacity-40 transition-opacity" />
                                                    ) : (
                    ...
                    <div className="sticky bottom-10 z-50 flex justify-end">
                        <button type="submit" disabled={loading || status.type === "success"} className="bg-white hover:bg-white/90 text-black shadow-2xl px-12 py-5 rounded-full uppercase text-xl flex items-center gap-3 transition-all active:scale-95 disabled:opacity-50" style={{ fontFamily: 'var(--font-bebas-neue)' }}>
                            {loading ? <><Loader2 className="w-6 h-6 animate-spin text-black/50" /><span>KAYDEDİLİYOR...</span></> : <><Save className="w-6 h-6" /><span>DEĞİŞİKLİKLERİ KAYDET</span></>}
                        </button>
                    </div>
                </form>
            </main>
